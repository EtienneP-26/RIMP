...
```bash
git switch -c <type/description-courte-en-kebab-case> <branche-base>
```

Exemple:
```bash
git switch -c feat/parsing-config-file dev
```
...
Pour travailler/tester dans un environnement Rust propre (image officielle `rust:1`) :
```bash
docker compose run --rm env
```
Ça ouvre un shell dans le container, projet monté sur `/app` (`cargo build`, `cargo test`, ...). Le registry cargo et le dossier `target/` sont dans des volumes Docker pour éviter de recompiler à chaque fois et de polluer le dossier local avec des fichiers root.

Pour lancer directement une commande :
```bash
docker compose run --rm env cargo test
```
